...
def decode_binary_entries(entries, datetimes,
                          x_offset, x_gain,
                          y_offset, y_gain,
                          z_offset, z_gain,
                          lux_ref, temp, volts):
    ...
    for idx, entry in enumerate(entries):
        x, y, z, lux, button, _ = entry
        xx = round((BitArray(bin=x.decode()).int * 100.0 - x_offset) / x_gain, 4)
        yy = round((BitArray(bin=y.decode()).int * 100.0 - y_offset) / y_gain, 4)
        zz = round((BitArray(bin=z.decode()).int * 100.0 - z_offset) / z_gain, 4)
        luz = int(lux.decode(), 2) * lux_ref / volts
        
        decoded.append((datetimes[idx], xx, yy, zz, luz, temp))
        
    return np.array(decoded, dtype=[
        ('Time', 'datetime64[ns]'),
        ('X', 'f8'),
        ('Y', 'f8'),
        ('Z', 'f8'),
...
def bin2df(full_path):
    ...
    with open(full_path, "rb") as in_file:
        full_lines = in_file.readlines()
        count = 0
        fs = ""
        df_data = None
        
        print("Total number of lines in file: %i" % len(full_lines))
        ...
        for full_line in full_lines:
            ...
            if len(line) == 3600:
                ...
                datetimes = (np.array([time]*300) + delta).to_numpy()

                st = datetime.datetime.now()
                decoded_entries = decode_binary_entries(bins, datetimes,
                                                        x_offset, x_gain,
                                                        y_offset, y_gain,
                                                        z_offset, z_gain,
                                                        lux, temp, volts)
                ...
                # decode["T"] = temp
                # df_data.append(decode)
                
                if df_data is None:
                    df_data = decoded_entries
                else:
                    df_data = np.vstack((df_data, decoded_entries))
